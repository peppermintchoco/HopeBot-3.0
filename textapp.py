...
async def text_to_speech_async(text):
    ...
    with ThreadPoolExecutor() as pool:
        response = await loop.run_in_executor(pool, lambda: openai.audio.speech.create(model="tts-1", voice="coral", input=text))
